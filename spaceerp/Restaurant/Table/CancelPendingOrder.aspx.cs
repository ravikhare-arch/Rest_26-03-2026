...
public partial class CancelPendingOrderlist : System.Web.UI.Page
{
    ...
    {
        string apiURL = clsConfiguration.ApiUrl + "/api/Item/CancelledPendingOrder";
        //string apiURL = "http://localhost:5000/api/Item/CancelledPendingOrder";
        string startDate = txttLastPurchase.Text;
        string endDate = txttLastOrder.Text;
        string orderTypeName =ddlordertype.SelectedValue;
        string urlParameters = "?orderType=" + orderTypeName + "&startDate=" + startDate + "&endDate=" + endDate;
        ...
        try
        {
            ...
            #region start code
            var fileName = "Cancelled Pending Order Report - " + DateTime.Now.ToString("yyyy-MM-dd--hh-mm-ss") + ".xlsx";
            var outputDir = Server.MapPath("../Temp/Cancelled Pending Order.xlsx");
            ...
            using (var package = new ExcelPackage(file))
            {
                ...
                ExcelWorksheet worksheet = package.Workbook.Worksheets.Add("Cancelled Pending Order - " + DateTime.Now.ToShortDateString());
                ...
                worksheet.Cells[4, 1, 4, 13].Merge = true; //Merge columns start and end range
                ...
                worksheet.Cells[1, 1, 1, 13].Merge = true; //Merge columns start and end range
                worksheet.Cells[1, 1, 1, 13].Style.Font.Bold = true; //Font should be bold
                worksheet.Cells[1, 1, 1, 13].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center; // Aligmnet is center
                worksheet.Cells[1, 1].Style.Font.Color.SetColor(System.Drawing.Color.WhiteSmoke);
                // // //im giving 13 for range the columns for header
                ///////  //Merging cells and create a center heading for out table
                worksheet.Cells[3, 1].Value = "	Statement  Details From : '" + txttLastPurchase.Text + "' To: '" + txttLastOrder.Text + "' "; // Heading Name               
                worksheet.Cells[3, 1, 3, 13].Merge = true; //Merge columns start and end range
                worksheet.Cells[3, 1, 3, 13].Style.Font.Bold = true; //Font should be bold
                worksheet.Cells[3, 1, 3, 13].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center; // Aligmnet is center
                ...
                ///////  //Merging cells and create a center heading for out table
                worksheet.Cells[4, 1].Value = "	Statement Name : Cancelled Pending Order"; // Heading Name               
                worksheet.Cells[4, 1, 4, 13].Merge = true; //Merge columns start and end range
                worksheet.Cells[4, 1, 4, 13].Style.Font.Bold = true; //Font should be bold
                worksheet.Cells[4, 1, 4, 13].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center; // Aligmnet is center
                ...
                //worksheet.Cells[5, 1, 5, 13].Merge = true; //Merge columns start and end range
                //worksheet.Cells[5, 1, 5, 13].Style.Font.Bold = true; //Font should be bold
                //worksheet.Cells[5, 1, 5, 13].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center; // Aligmnet is center
                ...
                using (var range = worksheet.Cells[6, 1, 6, 13])
                {
                ...
                for (int j = 0; j < orders.Count; j++)
                {
                    ...
                    worksheet.Cells[6, 6].Value = "Customer";
                    worksheet.Cells[6, 7].Value = "Table Status";
                    worksheet.Cells[6, 8].Value = "Total Amount";
                    worksheet.Cells[6, 9].Value = "Total Discount";
                    worksheet.Cells[6, 10].Value = "Charge";
                    worksheet.Cells[6, 11].Value = "Total GST";
                    worksheet.Cells[6, 12].Value = "Total Paid";
                    worksheet.Cells[6, 13].Value = "Payment Status";
                }
                ...
                foreach (var item in orders)
                {
                    ...
                    worksheet.Cells["F" + (i + 7)].Value = item.CustomerName;
                    worksheet.Cells["G" + (i + 7)].Value = item.TableStatus;
                    worksheet.Cells["H" + (i + 7)].Value = item.TotalOrderAmount;
                    worksheet.Cells["I" + (i + 7)].Value = item.TotalDiscount;
                    worksheet.Cells["J" + (i + 7)].Value = item.Charge;
                    worksheet.Cells["K" + (i + 7)].Value = item.GSTCost;
                    worksheet.Cells["L" + (i + 7)].Value = item.TotalPaid;
                    worksheet.Cells["M" + (i + 7)].Value = item.PaymentStatus;
                    count++;
                }
                ...
                Response.AddHeader("content-disposition", "attachment;filename=Cancelled Pending Order.xlsx");
