<%@ Page Title="Table Master" Language="C#" MasterPageFile="~/Pagecontent.master" AutoEventWireup="true" CodeFile="TableMaster.aspx.cs" Inherits="TableMaster" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="panel panel-inverse">
        <div class="panel-heading">
            ...
            <h4 class="panel-title text-center">Table Master </h4>
                ...
                <div class="modal-dialog modal-sm" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLongTitle">Table Master</h5>
                                                ...
                                                <div class="panel-body">
                                                    <asp:UpdatePanel ID="UpdatePanel3" runat="server">
                                                        <ContentTemplate>
                                                            ...
                                                            <div class="form-group row">
                                                                <label for="form-1-3" class="col-md-4 control-label">Table Name</label>
                                                                <div class="col-md-8">
                                                                    <asp:TextBox ID="txttablename" CssClass="form-control" runat="server"></asp:TextBox>

                                                                </div>

                                                            </div>
                                                            <div class="form-group row">
                                                                <label for="form-1-3" class="col-md-4 control-label">Capacity</label>
                                                                <div class="col-md-4">
    ...
    <div class="modal fade" id="deleteModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-keyboard="false">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content" style="background: #C0C0C0">
                <div class="modal-header">
                    <h5 class="modal-title" id="delModalLongTitle" style="color: white">Table Master</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    <script type="text/javascript">
        ...
        $(document.body).ready(function () {
            ...
            $("#btnadd").on("click", function () {
                if (validatedata()) {
                    var obj = {
                        TableName: $("#ctl00_ContentPlaceHolder1_txttablename").val(),
                        AreaID: $("#ctl00_ContentPlaceHolder1_ddlarea").val(),
                        Capacity: $("#ctl00_ContentPlaceHolder1_txtcapacity").val(),
                        OrderTypeID: $("#ctl00_ContentPlaceHolder1_ddlordertype").val(),
                    }

                    $.ajax({
                        url: '<%=ResolveUrl("TableMaster.aspx/addPagePamster") %>',
            ...
            $(document.body).on("click", "#deletebutton", function () {
                ...
                $.ajax({
                    type: "POST",
                    url: '<%=ResolveUrl("TableMaster.aspx/DeletePageMaster") %>',
            ...
            $(document.body).on("click", ".editbtn", function () {
                ...
                $("#ctl00_ContentPlaceHolder1_txttablename").val($(this).closest('tr').find('td:eq(1)').text());
                $("#ctl00_ContentPlaceHolder1_ddlarea").val($(this).closest('tr').find('td:eq(3)').text());
                $("#ctl00_ContentPlaceHolder1_txtcapacity").val($(this).closest('tr').find('td:eq(4)').text());
                $("#ctl00_ContentPlaceHolder1_ddlordertype").val($(this).closest('tr').find('td:eq(3)').text());
                $('#btnupdate').show();
            ...
            $(document.body).on("click", "#btnupdate", function () {
                ...
                $.ajax({
                    url: '<%=ResolveUrl("TableMaster.aspx/UpdatePageMaster") %>',
                    data: JSON.stringify({ list: obj }),
                    type: "post",
                    contentType: "application/json;charset=utf-8",
                    dataType: "json",
        ...
        function validatedata() {
            var ledgername = document.getElementById("<%=txttablename.ClientID %>").value;
        ...
        function loaddata() {
            ...
            $.ajax({
                url: '<%=ResolveUrl("TableMaster.aspx/loaddata") %>',
                type: "post",
                contentType: "application/json;charset=utf-8",
                dataType: "json",
                success: function (mainlist) {
                    var html = "<table id='tblagentlist' class='table table-striped table-bordered' style='width:100%'><thead><tr> <td style='width:2%'>Sr No.</td><td>Table Name</td><td>Capacity</td><td >Area Name</td><td >Order Type Name</td><td style='width:3%'>Edit/Delete</td><td style='display:none'>Account LedgerID</td><td style='display:none'>Area ID</td></tr></thead><tbody>";
                    var vcount = 0;
                    for (i = 0; i < mainlist.d.mpagemasterobjlist.length; i++) {
                        ...
                        html += '<td >' + mainlist.d.mpagemasterobjlist[i].Capacity + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlist[i].AreaName + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlist[i].OrderTypeName + '</td>';
