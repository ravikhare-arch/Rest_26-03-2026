<%@ Page Title="Item Wise Closing Sales Report" Language="C#" MasterPageFile="~/Pagecontent.master" AutoEventWireup="true" CodeFile="ItemWiseClosingSalesReport.aspx.cs" Inherits="ItemWiseClosingSalesReport" %>
...
<div class="col-md-2">
   ...
   <script type="text/javascript">
       ...
       // Function to convert HTML5 date string to SQL compatible format
       ...
       function loaddata() {
           ...
           // HTML5 date picker se value nikalna
           var valStart = $("#fromdate").val();
           var valEnd = $("#todate").val();

           if (!valStart || !valEnd) {
               alert("Bhai, please dono dates select karo!");
               return;
           }

           var startDate = convertstringtodate(valStart);
           var endDate = convertstringtodate(valEnd);

           $.ajax({
               url: apiUrl + '/api/Item/ItemWiseClosingSalesOrder?startDate=' + startDate + '&endDate=' + endDate,
               type: "GET",
               contentType: "application/json;charset=utf-8",
               dataType: "json",
               success: function (data) {
                   var vcount = 0;
                   var html = "<table id='tblagentlist' class='table table-striped table-bordered table-responsive' style='width:100%'><thead><tr><td style='width:2%'>Sr No.</td><td>Item Name</td><td>Product Qty</td><td>CGST</td><td>SGST</td><td>Grand Total</td></tr></thead><tbody>";
                   ...
                   html += "</tbody><tfoot><tr class='table table-striped table-bordered table-responsive'><th colspan='2' style='text-align:right;'> Total</th><th> </th><th> </th><th> </th><th> </th></tr></tfoot></table>";
                   ...
                   $("#tblagentlist").DataTable({
                       ...
                       "footerCallback": function (row, data, start, end, display) {
                           ...
                           // Column wise totals
                           var qtyTotal = api.column(2).data().reduce(function (a, b) { return intVal(a) + intVal(b); }, 0);
                           var sgstTotal = api.column(3).data().reduce(function (a, b) { return intVal(a) + intVal(b); }, 0);
                           var cgstTotal = api.column(4).data().reduce(function (a, b) { return intVal(a) + intVal(b); }, 0);
                           var grandTotal = api.column(5).data().reduce(function (a, b) { return intVal(a) + intVal(b); }, 0);

                           $(api.column(2).footer()).html(qtyTotal);
                           $(api.column(3).footer()).html(sgstTotal.toFixed(2));
