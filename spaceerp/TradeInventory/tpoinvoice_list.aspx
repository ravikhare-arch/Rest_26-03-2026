<%@ Page Title="Purchase Invoice" Language="C#" MasterPageFile="~/Pagecontent.master" AutoEventWireup="true" CodeFile="tpoinvoice_list.aspx.cs" Inherits="tpoinvoice_list" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="panel panel-inverse">
        <div class="panel-heading">
            ...
            <div class="panel-heading-btn pull-left" id="divpanelheadingprint">

                <asp:LinkButton ID="lnkAdd" runat="server" OnClick="lnkAdd_Click" CssClass="btn btn-info btn-xs">ADD</asp:LinkButton>

            </div>
            <div class="panel-heading-btn" id="divpanelheadingprintmax">
                <a href="javascript:;" class="btn btn-xs btn-icon btn-circle btn-default" data-click="panel-expand"><i class="fa fa-expand"></i></a>
                <a href="javascript:;" class="btn btn-xs btn-icon btn-circle btn-warning" data-click="panel-collapse"><i class="fa fa-minus"></i></a>
            </div>

            <h4 class="panel-title text-center">Purchase Invoice </h4>
        ...
        <div class="panel-body">
            ...
            <div class="col-md-12 col-md-push-1" id="divdisplaysearch">
        ...
        <div class="invoice" id="scndgrddiv" runat="server">
            ...
            <div class="invoice-company text-inverse f-w-600">
                ...
                <div class="text-center" style="text-align: center">
                    <h4 style="font-family: Calibri;"><b>Ledger Name :- Purchase Invoice  </b></h4>
                    <h4>
                ...
                <div class="table-responsive">
                    ...
                    <asp:GridView ID="GridViewexcel" CssClass="table" runat="server" AutoGenerateColumns="False"
                        ...
                        <Columns>
                            ...
                            <asp:BoundField DataField="nShipingCost" HeaderText="Shipping Cost" />
    ...
    <script type="text/javascript">
        ...
        $(document.body).ready(function () {
            //Code for printing HotelInvoice
            <%--$("#<%=btnprint.ClientID%>").click(function (e) {
                $("#<%=btnexcel.ClientID%>").hide();
                $("#<%=btnsendmail.ClientID%>").hide();
                $("#<%=btnpdf.ClientID%>").hide();
                $("#<%=btnprint.ClientID%>").hide();
                $('#divpanelheadingprint').hide();
                $('#divpanelheadingprintmax').hide();
                $('#divdisplaysearch').hide();
                $('#divprint').hide();
                $("#tblagentlist").DataTable({ searching: false });
                window.print();
            });--%>
            ...
            $(document.body).on("click", ".editbtn", function () {
                ...
                var id = $(this).closest('tr').find('td:eq(13)').text();
                ...
                $.ajax({
                    ...
                    success: function (html) {
                        window.location.href = "../TradeInventory/tpoinvoice.aspx?" + datastring + "";
                    }
                });
            });
            ...
            $(document.body).on("click", "#deletebutton", function () {
                ...
                $.ajax({
                    type: "POST",
                    url: '<%=ResolveUrl("tpoinvoice_list.aspx/DeleteVoucher") %>',
        ...
        function loaddata() {
            ...
            $.ajax({
                url: '<%=ResolveUrl("tpoinvoice_list.aspx/loaddata") %>',
                type: "post",
                data: JSON.stringify({ fromdate: fromdate, todate: todate, Reportfor: Reportfor }),
                contentType: "application/json;charset=utf-8",
                dataType: "json",
                success: function (mainlist) {
                    var html = "<table id='tblagentlist' class='table table-striped table-bordered table-responsive' style='width:100%'><thead><td style='width:2%'>Sr No.</td><td>Invoice No</td><td>Invoice Date</td><td>Vendor Name </td><td>Shiping Cost</td><td>Other Charges</td><td>Discount </td><td>Total Tax </td><td>Sub Total</td><td>Total Amount</td><td>Balance </td><td>Paid Status </td><td style='width:3%'>Edit/Delete</td><td style='display:none'>TicketID</td></tr></thead><tbody>";
                    var vcount = 0;
                    for (i = 0; i < mainlist.d.mpagemasterobjlist.length; i++) {
                        ...
                        html += '<td >' + mainlist.d.mpagemasterobjlist[i].ShippingCost + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlist[i].OtherCharges + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlist[i].Discount + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlist[i].TotalTax + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlist[i].Subtotal + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlist[i].TotalAmount + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlist[i].Balance + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlist[i].PaidStatus + '</td>';
                        html += '<td >' + '<a href="tpoinvoice.aspx" class="editbtn" title="Edit"><i class="glyphicon glyphicon-edit" style="color: green" data-toggle="modal" data-target="#exampleModalCenter"></i></a>&nbsp;&nbsp;&nbsp;<a href="#" class="deletebtn" title="Delete" data-toggle="modal" data-target="#deleteModalCenter"><i class="glyphicon glyphicon-trash" style="color: red"></i></a>' + '</td>';
                        html += '<td style="display:none;" >' + mainlist.d.mpagemasterobjlist[i].PoInvoiceID + '</td>';
                        html += '</tr>';
                    }
