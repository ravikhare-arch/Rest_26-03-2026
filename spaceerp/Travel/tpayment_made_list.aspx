<%@ Page Title="Supplier Payment Voucher" Language="C#" MasterPageFile="~/Pagecontent.master" AutoEventWireup="true" CodeFile="tpayment_made_list.aspx.cs" Inherits="tpayment_made_list" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function pageLoad(sender, args) {
            $(document.body).ready(function () {
                ...
                $(document.body).on("click", ".editbtn", function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "tpayment_made.aspx",
                        data: datastring,
                        cache: false,
                        success: function (html) {
                            window.location.href = "../Travel/tpayment_made.aspx?" + datastring + "";
                        }
                    });
                });
        ...
        function loaddata() {
            ...
            $.ajax({
                url: '<%=ResolveUrl("tpayment_made_list.aspx/loaddata") %>',
                type: "post",
        ...
        function loadDetdata() {
            var subagentid = $("#accountledgerid").val();
            $.ajax({
                url: '<%=ResolveUrl("tpayment_receive_list.aspx/loaddetdata") %>',
                type: "post",
                data: JSON.stringify({ AccountLedgerID: subagentid }),
                contentType: "application/json;charset=utf-8",
                dataType: "json",
                success: function (mainlist) {
                    var html = "<table id='tblagentlistdet' class='table table-striped table-bordered table-responsive' style='width:100%'><thead><tr> <th style='width:2%'>Sr No.</th><th>Invoice Date</th><th>Invoice No</th><th>Selling Rate</th><th>Due Amount</th><th>Payment Value</th></tr></thead><tbody>";
                    var vcount = 0;
                    for (i = 0; i < mainlist.d.mpagemasterobjlistnew.length; i++) {
                        ...
                        html += '<tr>';
                        html += '<td >' + vcount + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlistnew[i].InvoiceDate + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlistnew[i].InvoiceNo + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlistnew[i].SellingRate + '</td>';
                        html += '<td >' + mainlist.d.mpagemasterobjlistnew[i].Balance + '</td>';
                       // html += '<td >' + mainlist.d.mpagemasterobjlistnew[i].PaymentValue + '</td>';
                        html += '<td >' + '<input type="text" value="' + mainlist.d.mpagemasterobjlistnew[i].PaymentValue + '"/>' + '</td>';
                       // html += '<td style="display:none;">'<input class="form-control" id="Comments" name="Comments" maxlength="350" multiline="true" readonly="true" type="text"  value =' + row.Comments + '>';</td>';
