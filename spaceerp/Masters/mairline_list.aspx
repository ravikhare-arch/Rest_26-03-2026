<%@ Page Title="Airlines" Language="C#" MasterPageFile="~/Pagecontent.master" AutoEventWireup="true" CodeFile="mairline_list.aspx.cs" Inherits="mairline_list" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="panel panel-inverse">
        <div class="panel-heading">
            ...
            <h4 class="panel-title text-center">Airlines</h4>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Update Airlines</h5>
                            ...
                            <div class="row">
                                <div class="col-md-9 ml-auto mr-auto">
                                    <div class="form-horizontal mrg-top-40 pdd-right-30 ng-pristine ng-valid">
                                        ...
                                        <div class="form-group row">
                                            <label for="form-1-3" class="col-md-4 control-label">Airline Name</label>
                                            <div class="col-md-8">
                                                <asp:TextBox ID="txtAirlineName" CssClass="form-control" runat="server"></asp:TextBox>
                                            </div>
                                        </div>
                                        <div class="form-group row" id="divpassword">
                                            <label for="form-1-3" class="col-md-4 control-label">Designator</label>
                                            <div class="col-md-8">
                                                <asp:TextBox ID="txtDesignator" CssClass="form-control" runat="server"></asp:TextBox>
                                            </div>
    ...
    <script type="text/javascript">
        ...
        $(document.body).ready(function () {
            ...
            $("#btnadd").on("click", function () {
                ...
                if (validatedata()) {
                    var obj = {
                        ...
                        AirlineName: $("#ctl00_ContentPlaceHolder1_txtAirlineName").val(),
                        Designator: $("#ctl00_ContentPlaceHolder1_txtDesignator").val(),
                    }

                    $.ajax({
                        url: '<%=ResolveUrl("mairline_list.aspx/AddAirline") %>',
            ...
            $(document.body).on("click", "#deletebutton", function () {
                ...
                $.ajax({
                    type: "POST",
                    url: '<%=ResolveUrl("mairline_list.aspx/DeleteAirline") %>',
            ...
            $(document.body).on("click", "#inactivebutton", function () {
                ...
                $.ajax({
                    type: "POST",
                    url: '<%=ResolveUrl("mairline_list.aspx/MarkInactive") %>',
            ...
            $(document.body).on("click", ".editbtn", function () {
                ...
                var tr = $(this).closest('tr td');
                var id = $(this).closest('tr').find('td:eq(5)').text();
                var CAid = $(this).closest('tr').find('td:eq(6)').text();
                $("#accountledgerid").val(id);
                $("#CAccountID").val(CAid);

                $("#btnadd").css("display", "none");
                $("#ctl00_ContentPlaceHolder1_txtCode").val($(this).closest('tr').find('td:eq(1)').text());
                $("#ctl00_ContentPlaceHolder1_txtAirlineName").val($(this).closest('tr').find('td:eq(2)').text());
                $("#ctl00_ContentPlaceHolder1_txtDesignator").val($(this).closest('tr').find('td:eq(3)').text());
            ...
            $(document.body).on("click", "#btnupdate", function () {
                var obj = {
                    ...
                    AirlineName: $("#ctl00_ContentPlaceHolder1_txtAirlineName").val(),
                    Designator: $("#ctl00_ContentPlaceHolder1_txtDesignator").val(),
                    AirlineID: $("#accountledgerid").val(),
                    CAccountID: $("#CAccountID").val(),
                }

                $.ajax({
                    url: '<%=ResolveUrl("mairline_list.aspx/UpdateAirline") %>',
                    data: JSON.stringify({ list: obj }),
                    type: "post",
                    contentType: "application/json;charset=utf-8",
                    dataType: "json",
        ...
        function cleardata() {
            ...
            $("#ctl00_ContentPlaceHolder1_txtAirlineName").val(''),
            $("#ctl00_ContentPlaceHolder1_txtDesignator").val('')

        }
        function validatedata() {
            var ledgername = document.getElementById("<%=txtCode.ClientID %>").value;
            var ledgercode = document.getElementById("<%=txtAirlineName.ClientID %>").value;
            var undergroup = document.getElementById("<%=txtDesignator.ClientID %>").value;
        ...
        function loaddata() {
            ...
            $.ajax({
                url: '<%=ResolveUrl("mairline_list.aspx/loaddata") %>',
                type: "post",
                contentType: "application/json;charset=utf-8",
                dataType: "json",
                success: function (mainlist) {
                    var html = "<table id='tblagentlist' class='table table-striped table-bordered' style='width:100%'><thead><tr> <td style='width:2%'>Sr No.</td><td>AIRLINE CODE</td><td>AIRLINE NAME</td><td>DESIGNATOR</td><td style='width:3%'>Edit/Delete</td><td style='display:none'>AirlineID</td><td style='display:none'>CAID</td></tr></thead><tbody>";
                    var vcount = 0;
                    for (i = 0; i < mainlist.d.muserobjlist.length; i++) {
                        ...
                        html += '<td >' + mainlist.d.muserobjlist[i].AirlineName + '</td>';
                        html += '<td >' + mainlist.d.muserobjlist[i].Designator + '</td>';
                        ...
                        html += '<td style="display:none;" >' + mainlist.d.muserobjlist[i].AirlineID + '</td>';
